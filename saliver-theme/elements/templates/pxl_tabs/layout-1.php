...
if(isset($settings['tabs']) && !empty($settings['tabs']) && count($settings['tabs'])): 
    $tab_bd_ids = [];
    $style_l2 = isset($settings['style_l2']) ? $settings['style_l2'] : ''; 
    ?>
    <div class="pxl-tabs pxl-tabs1 <?php echo esc_attr($settings['style_l2'].' '.$settings['tab_effect'].' '.$settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms">
                        ...
                        <div class="pxl-tab--title pxl-cursor--cta <?php if($settings['tab_active'] == $key + 1) { echo 'active'; } ?>" data-target="#<?php echo esc_attr($html_id.'-'.$title['_id']); ?>">
                            <?php if ( !empty($title['pxl_icon']) ) :
                                if ( $is_new ) {
                                    ...
                                    echo '<i class="' . esc_attr( $title['pxl_icon'] ) . '" aria-hidden="true"></i>';
                                }
                            endif; ?>
