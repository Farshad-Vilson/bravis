...
if(isset($settings['tabs']) && !empty($settings['tabs']) && count($settings['tabs'])): 
    $tab_bd_ids = [];
    ?>
    <div class="pxl-tabs pxl-tabs4 <?php echo esc_attr($settings['style_l2'].' '.$settings['tab_effect'].' '.$settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms">
                ...
                <div class="pxl-tabs--title">
                    ...
                    <?php foreach ($settings['tabs'] as $key => $title) :  
                        ...
                        <div class="pxl-tab--title pxl-cursor--cta <?php if($settings['tab_active'] == $key + 1) { echo 'active'; } ?>" data-target="#<?php echo esc_attr($html_id.'-'.$title['_id']); ?>">
                            <?php if ( $is_new ):
                                \Elementor\Icons_Manager::render_icon( $title['pxl_icon'], [ 'aria-hidden' => 'true' ] );
                                elseif(!empty($title['pxl_icon'])): ?>
                                <i class="<?php echo esc_attr( $title['pxl_icon'] ); ?>" aria-hidden="true"></i>
                            <?php endif; ?>
                            <h5 class="pxl-title--text pxl-ml-12">
