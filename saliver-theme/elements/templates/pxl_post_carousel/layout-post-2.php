...
<?php if (is_array($posts)): ?>
    <div class="pxl-swiper-slider pxl-post-carousel pxl-post-carousel2 <?php echo esc_attr($settings['style']); ?>" <?php if($settings['drap'] !== false) : ?>data-cursor-drap="<?php echo esc_attr__('DRAG', 'saliver'); ?>"<?php endif; ?>>
                                            ...
                                            <?php
                                                ...
                                                if (!empty($categories)): 
                                                    ...
                                                    foreach ($categories as $category) {
                                                        $category_links[] = '<span class="filter-item" data-filter="'. esc_attr('.' . $category->slug) .'">' . esc_html($category->name) . '</span>';
                                                    }
                                                    echo implode(', ', $category_links);
                                                endif;
                                            ?>
                                            <?php if ( $show_date === 'true' ): ?>
                                                <span class="pxl-date pxl-ml-20"><?php echo get_the_date( 'F j, Y', $post->ID ); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="pxl-post--title"><a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php echo esc_html(get_the_title($post->ID)); ?></a></div>
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
