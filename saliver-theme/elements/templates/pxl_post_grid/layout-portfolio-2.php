...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-grid">'.esc_html__( 'No Post Found', 'saliver' ). '</div>';
    return;
} ?>

<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?>>
    <?php if ($settings['hover_style'] == 'hover-image-transition'): ?>
    ...
    <?php if ($select_post_by == 'term_selected' && $filter == "true"): ?>
        <div class="pxl-grid-filter <?php echo esc_attr($filter_type) ?> style-1">
            <div class="pxl--filter-inner">
                <span class="filter-label"><?php echo esc_html__('Filter By :', 'saliver'); ?></span>
    ...
    <div class="<?php echo esc_attr($grid_class); ?>" data-gutter="15" data-layout="<?php echo esc_attr($layout_mode); ?>">
        <?php saliver_get_post_grid($posts, $load_more); ?>
        <div class="grid-sizer <?php echo esc_attr($grid_sizer); ?>"></div>
        
    </div>
    <?php if ($pagination_type == 'pagination') { ?>
        <div class="pxl-grid-pagination">
            <?php saliver()->page->get_pagination($query, true); ?>
        </div>
    <?php } ?>
    <?php if (!empty($next_link) && $pagination_type == 'loadmore') { ?>
        <div class="pxl-load-more">
            <span class="btn btn-grid-loadmore">
                <span class="pxl-loadmore-text"><?php echo esc_html__('Load More', 'saliver') ?></span>
