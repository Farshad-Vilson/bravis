...
if (!class_exists('Saliver_Page')) {

    class Saliver_Page
    {
        public function get_site_loader(){
            $site_loader = saliver()->get_theme_opt( 'site_loader', 'off' );
            $site_loader_style = saliver()->get_theme_opt( 'site_loader_style', 'style-1' );
            $site_loader_icon = saliver()->get_theme_opt( 'site_loader_icon' );
            if($site_loader == 'on') { ?>
        ...
        public function get_page_title(){
            ...
            $pt_mode = saliver()->get_opt('pt_mode');
            $pt_mode_product = saliver()->get_opt('pt_mode_product');
            $ptitle_scroll_opacity = saliver()->get_opt('ptitle_scroll_opacity');
            $ptitle_layout = (int)saliver()->get_opt('ptitle_layout');
            $ptitle_layout_product = (int)saliver()->get_opt('ptitle_layout_product');
            ...
            <?php } elseif($pt_mode == 'df') {
                $ptitle_breadcrumb_on = saliver()->get_opt( 'ptitle_breadcrumb_on', '1' ); 
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <h1 class="pxl-page-title"><?php echo saliver_html($titles['title']) ?></h1>
                            </div>
        ...
        public function get_title() {
            ...
            if ( ! is_archive() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404 Error', 'saliver' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'saliver' );
                } elseif ( is_singular('lp_course') ) {
                    $title = esc_html__( 'Course', 'saliver' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                    if ( ! $title ) {
                        $title = get_the_title();
        ...
        public function get_breadcrumb(){
            ...
            foreach ( $entries as $entry )
            {
                ...
                {
                    printf(
                        '<a class="breadcrumb-hidden" href="%1$s">%2$s<i aria-hidden="true" class="fal fa-long-arrow-right"></i></a>',
                        esc_url( $entry['url'] ),
                ...
                else
                {
                    ...
                    $sg_post_title_text = saliver()->get_theme_opt('sg_post_title_text');
                    if(is_singular('post') && $sg_post_title == 'custom_text' && !empty($sg_post_title_text)) {
                        $entry_label = $sg_post_title_text;
                    }

                    $sg_portfolio_title = saliver()->get_theme_opt('sg_portfolio_title', 'default');
                    $sg_portfolio_title_text = saliver()->get_theme_opt('sg_portfolio_title_text');
                    if(is_singular('portfolio') && $sg_portfolio_title == 'custom_text' && !empty($sg_portfolio_title_text)) {
                        $entry_label = $sg_portfolio_title_text;
                    }

                    $sg_service_title = saliver()->get_theme_opt('sg_service_title');
                    $sg_service_title_text = saliver()->get_theme_opt('sg_service_title_text');
                    if(is_singular('service') && $sg_service_title == 'custom_text' && !empty($sg_service_title_text)) {
                        $entry_label = $sg_service_title_text;
                    }
                    
                    $sg_product_ptitle = saliver()->get_theme_opt('sg_product_ptitle', 'default');
                    $sg_product_ptitle_text = saliver()->get_theme_opt('sg_product_ptitle_text');
                    if(is_singular('product') && $sg_product_ptitle == 'custom_text' && !empty($sg_product_ptitle_text)) {
                        $entry_label = $sg_product_ptitle_text;
                    }

                    $sg_career_title = saliver()->get_theme_opt('sg_career_title', 'default');
                    $sg_career_title_text = saliver()->get_theme_opt('sg_career_title_text');
                    if(is_singular('career') && $sg_career_title == 'custom_text' && !empty($sg_career_title_text)) {
                        $entry_label = $sg_career_title_text;
                    }
        ...
        public function get_pagination( $query = null, $ajax = false ){
            ...
            if($ajax){
                add_filter('paginate_links', 'saliver_ajax_paginate_links');
            }
