...
class Saliver_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = saliver()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'saliver_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'saliver_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->saliver_generate_file_options();
		} );
	}

	function saliver_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->saliver_generate_file_options();
			$this->saliver_generate_file();
		}
	}

    function saliver_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->saliver_options_output() )
        ) );
    }

	function saliver_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function saliver_options_output() {
		$theme_colors                    = saliver_configs('theme_colors');
        $links                           = saliver_configs('link');
        $gradients                       = saliver_configs('gradient');
        $gradients2                      = saliver_configs('gradient2');
		ob_start();

		printf('$saliver_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$saliver_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
        ...
        /* Font */
        $font_secondary = saliver()->get_theme_opt('f_secondary');
    ...
    /* Inline CSS */
    function saliver_enqueue() {
        $css = $this->saliver_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function saliver_inline_css() {
        ob_start();
