...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'saliver');
...
$merlin_setup = get_option( 'merlin_' . saliver()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'saliver');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'saliver');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'saliver');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'saliver' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'saliver' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'saliver' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Saliver_Admin_Plugins;
