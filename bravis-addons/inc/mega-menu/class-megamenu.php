...
class PXL_MegaMenu_Register
{
    ...
    function update_nav_menu_item($menu_id, $menu_item_db_id, $menu_item_data)
    {
        ...
        if (isset($_REQUEST['menu-item-pxl-onepage'][$menu_item_db_id])) {
            $onepage_val = sanitize_text_field(wp_unslash($_REQUEST['menu-item-pxl-onepage'][$menu_item_db_id]));
            if (in_array($onepage_val, array('is-one-page', ''), true)) {
                update_post_meta($menu_item_db_id, '_menu_item_pxl_onepage', $onepage_val);
            }
        }

        if (isset($_REQUEST['menu-item-pxl-onepage-offset'][$menu_item_db_id])) {
            update_post_meta($menu_item_db_id, '_menu_item_pxl_onepage_offset', sanitize_text_field(wp_unslash($_REQUEST['menu-item-pxl-onepage-offset'][$menu_item_db_id])));
        }

        foreach ($this->menu_meta_extra as $key => $fields) {
            if (isset($_REQUEST['menu-item-' . $key][$menu_item_db_id])) {
                update_post_meta($menu_item_db_id, '_menu_item_' . $key, sanitize_text_field(wp_unslash($_REQUEST['menu-item-' . $key][$menu_item_db_id])));
            }
        }
    }
